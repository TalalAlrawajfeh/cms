...
public class CopyUtil {
    ...
    public static <D, S> void copyFields(D destination, S source, CopySetting[] copySettings, Class<?>[] discardClasses,
        ...
        Stream.of(source.getClass().getDeclaredFields()).forEach(f -> handleField(destination, source, f, copySettings, discardClasses, depth));
    }

    private static <D, S> void handleField(D destination, S source, Field f, CopySetting[] copySettings, Class<?>[] discardClasses, int depth) {
    ...
    private static <S, D> void copyField(D destination, S source, Method getter, Method setter) throws InvocationTargetException, IllegalAccessException {
